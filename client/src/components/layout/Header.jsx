      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            ...
            <span className="text-lg font-bold text-black hidden sm:block">OPD EMR</span>
          </Link>
              ...
              <Link
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${
                  isActive(item.to)
                    ...
                    : 'text-black hover:text-primary-600 hover:bg-slate-50'
                }`}
              >
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={userMenuRef}>
              ...
              >
                <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center text-primary-600 font-semibold text-base">
                  {getInitials()}
                </div>
                ...
                {userMenuOpen ? (
                  <FaChevronUp className="text-black text-xs" />
                ) : (
              ...
              {userMenuOpen && (
                <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-soft-lg border border-slate-100 py-2 animate-fade-in">
                  {/* User Info Header */}
                  <div className="px-4 py-4 border-b border-slate-100">
                    <div className="flex items-center gap-3">
                      <div className="w-14 h-14 bg-primary-100 rounded-full flex items-center justify-center text-primary-600 font-semibold text-xl">
                        {getInitials()}
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-black">{user?.firstName} {user?.lastName}</p>
                        <p className="text-xs text-black truncate max-w-[160px]">{user?.email}</p>
                  ...
                  <div className="py-2">
                    <Link
                      to="/settings"
                      className="flex items-center gap-3 px-4 py-3 text-sm text-black hover:bg-primary-50 hover:text-primary-600 transition-colors"
